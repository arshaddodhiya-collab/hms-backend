...
public class PatientController {
    ...
    @PostMapping
    public ResponseEntity<PatientProjection> registerPatient(@Valid @RequestBody PatientRegisterRequest request) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<PatientDetailsProjection> getPatientDetails(@PathVariable Long id) {
        return ResponseEntity.ok(patientService.getPatientDetails(id));
    }

    @PutMapping("/{id}")
    public ResponseEntity<PatientProjection> updatePatient(@PathVariable Long id,
